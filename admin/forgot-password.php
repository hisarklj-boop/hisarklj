...
$error = '';
$success = '';
$step = isset($_SESSION['admin_forgot_step']) ? $_SESSION['admin_forgot_step'] : 'phone';
    ...
    if(empty($phone)) {
        ...
        try {
            ...
            if($user) {
                ...
                $stmt = $db->prepare("UPDATE users SET reset_otp = ?, otp_expiry = ? WHERE phone = ? AND role = 'admin'");
                $stmt->execute([$otp, $otp_expiry, $phone]);
                
                $_SESSION['admin_forgot_phone'] = $phone;
                $_SESSION['admin_forgot_step'] = 'otp';
                $_SESSION['admin_otp_time'] = time();
                $step = 'otp';
                $success = 'OTP kodu gönderildi! (Test: 111112)';
            } else {
                $error = 'Bu telefon numarası admin olarak kayıtlı değil!';
            }
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['verify_otp'])) {
    $otp = trim($_POST['otp']);
    $phone = $_SESSION['admin_forgot_phone'] ?? '';
    ...
    if(empty($otp)) {
        ...
        try {
            $stmt = $db->prepare("SELECT * FROM users WHERE phone = ? AND reset_otp = ? AND otp_expiry > NOW() AND role = 'admin'");
            $stmt->execute([$phone, $otp]);
            $user = $stmt->fetch();
            
            if($user) {
                $_SESSION['admin_forgot_step'] = 'newpassword';
                $_SESSION['admin_forgot_user_id'] = $user['id'];
                $step = 'newpassword';
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['reset_password'])) {
    ...
    $user_id = $_SESSION['admin_forgot_user_id'] ?? 0;
    ...
    if(empty($password) || empty($confirm_password)) {
        ...
        try {
            ...
            $stmt = $db->prepare("UPDATE users SET password = ?, reset_otp = NULL, otp_expiry = NULL WHERE id = ? AND role = 'admin'");
            $stmt->execute([$hashed_password, $user_id]);
            
            // Session temizle
            unset($_SESSION['admin_forgot_step']);
            unset($_SESSION['admin_forgot_phone']);
            unset($_SESSION['admin_forgot_user_id']);
            unset($_SESSION['admin_otp_time']);
            
            header('Location: /admin/login.php?success=Şifreniz başarıyla güncellendi!');
            exit;
        } catch(PDOException $e) {
            $error = 'Sistem hatası oluştu!';
        }
...
$remaining_time = 60;
if(isset($_SESSION['admin_otp_time'])) {
    $elapsed = time() - $_SESSION['admin_otp_time'];
...
<head>
    ...
    <title>Admin Şifremi Unuttum - Okul Kantini</title>
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="forgot-card w-full max-w-md rounded-2xl shadow-2xl p-8">
        <div class="text-center mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Admin Şifremi Unuttum</h1>
            <?php if($step == 'phone'): ?>
                <p class="text-gray-600">Admin telefon numaranızı girin</p>
            <?php elseif($step == 'otp'): ?>
        ...
        <form method="POST" action="" class="space-y-6">
            <div>
                <label class="label">
                    <span class="label-text font-semibold text-gray-700">Admin Telefon Numarası</span>
                </label>
                <div class="relative">
            ...
            <button type="submit" name="send_otp" class="btn btn-error w-full btn-lg">
        ...
        <div class="text-center mt-6">
            <a href="/admin/login.php" class="link link-primary">
